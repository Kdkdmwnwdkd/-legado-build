...
class ReadView(context: Context, attrs: AttributeSet) :
    ...
    fun fillPage(direction: PageDirection): Boolean {
        ...
        if (ok && AppConfig.readAloudFollowPage && BaseReadAloudService.isRun) {
            // 手动翻页后，让听书跟随到当前可见页继续朗读
            post { callBack.onReadAloudFollowPageTurn() }
        }
        return ok
    }
    ...
    interface CallBack {
        ...
        /**
         * 手动翻页完成且开启了“听书跟随翻页”时回调，用于让朗读跳到当前页继续
         */
        fun onReadAloudFollowPageTurn() {}
    }
}
