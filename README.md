...
- **安全修复**：默认启用 HTTPS 证书校验、关闭全局明文 HTTP 传输、WebView SSL 错误不再静默忽略
- **权限收缩**：移除不必要的权限声明（电话状态、安装包、外部存储管理）
- 注意：大站书源完全不受影响；小站书源需手动开启信任开关

#### 全部历史版本
[前往 GitHub Release 页面查看所有版本](https://github.com/Kdkdmwnwdkd/-legado-build/releases)
